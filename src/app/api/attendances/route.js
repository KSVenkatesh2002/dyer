...
export async function POST(req) {
    ...
    try {
        ...
        if (!validStatuses.includes(status)) {
            return NextResponse.json({ error: 'Invalid status. Must be "present", "half", "absent", or "holiday".' }, { status: 400 });
        }
        ...
        // Validate date
        const dateValidationResponse = validateAttendanceDate({ formattedDate, employee, summary });
        ...
        const existing = await Attendance.findOne({ employeeId, date: formattedDate, createdBy: userId });
        const attendanceValidationResponse = checkExistingAttendance(existing, status);
        ...
        handleRemoveChanges({ existing, summary, salaryPerDay: employee.salaryPerDay });

        const { paid, partialPay } = handleNewChange({ status, salary, summary, formattedDate });
