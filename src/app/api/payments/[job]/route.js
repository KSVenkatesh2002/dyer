...
export async function POST(req, { params }) {
  ...
  try {
    ...
    const { employeeId, amountPaid, method, paidFor, note, paidAt } =
      await req.json();

    const validatePaymentInputResponse = await validatePaymentInput({
      employeeId,
      amountPaid,
      job,
      method,
...
export async function GET(req) {
  ...
  try {
    ...
    if (!records || records.length === 0) {
      return NextResponse.json(
        { message: "No payment record found for this employee." },
        { status: 200 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Server error. Could not fetch payment record." },
      { status: 500 }
    );
  }
}
