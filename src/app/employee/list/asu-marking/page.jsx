...
import { getEmployeesListByJob, getProductsMarkingUnassigned, markingTask } from '@/lib/api';
import { toast } from 'react-toastify';
import { AiOutlineLoading3Quarters } from "react-icons/ai";
import Link from 'next/link';
import API from '@/lib/api'; // Ensure your axios instance is correctly imported
...
export default function AsuMarkingEmployeesList() {
    ...
    const fetchProducts = async (pageNum = page, reset = false) => {
        ...
        try {
            const res = await getProductsMarkingUnassigned(pageNum);
        ...
        <>
            <div className="py-8 max-w-3xl mx-auto w-full space-y-4 px-4">
                ...
                <ul className="space-y-4">
                    {employees.map(emp => (
                        <li key={emp._id} className="flex justify-between items-center p-3 rounded-lg shadow bg-white">
                            <Link href={`/dashboard/task-based/${emp._id}`} className="text-lg font-semibold capitalize">
                                {emp.name}
                            </Link>
